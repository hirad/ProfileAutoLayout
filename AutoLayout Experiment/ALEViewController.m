...
- (IBAction)moveContentView:(UIPanGestureRecognizer *)sender {
    CGFloat yOffset = [sender translationInView:self.view].y;
    
    switch (sender.state) {
        case UIGestureRecognizerStateBegan:
            [self animateWithTranslation:yOffset animated:NO];
            break;
        case UIGestureRecognizerStateChanged:
            [self animateWithTranslation:yOffset animated:NO];
            break;
        case UIGestureRecognizerStateEnded:
            yOffset < 0 ? : [self animateWithTranslation:0.0 animated:YES];
...
-(void)animateWithTranslation:(CGFloat)translation animated:(BOOL)animated {
...
//    self.headerHandleLabel.hidden = self.headerNameLabel.hidden = (nameLabelY < newHeaderHeight);
